...
- 第四步: [滑桿控制，入出分組與操作視聽設備](#第四滑桿控制步入出分組與操作視聽設備)
- 第五步: [廣播系統](#第五步廣播系統)
...
- 建議老師都用滑桿控制 （slider) 操作視聽的按鈕。如果老師在 meet 直接 meet 的按視聽按鈕（喇叭，麥克風，相機）meet 與控制面板會造成不一致，但這個是小問題。下次老師給 slider 移動，meet 與 slider 的視聽按鈕就會行程一致。
